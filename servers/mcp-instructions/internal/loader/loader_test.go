...
func newMockGitHubServer(t *testing.T) *httptest.Server {
	...
	mux.HandleFunc("/repos/testowner/testrepo/contents/.github", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode([]github.ContentEntry{
			{Name: "copilot-instructions.md", Path: ".github/copilot-instructions.md", Type: "file"},
			{Name: "instructions", Path: ".github/instructions", Type: "dir"},
		})
	})
...
func TestLoaderSyncRepoWithRef(t *testing.T) {
	...
	mux.HandleFunc("/repos/owner/repo/contents/", func(w http.ResponseWriter, r *http.Request) {
		...
		if ref != "develop" {
			t.Errorf("expected ref=develop for root, got %q", ref)
		}
		json.NewEncoder(w).Encode([]github.ContentEntry{
			{Name: ".github", Path: ".github", Type: "dir"},
		})
	...
	// .github dir listing
	mux.HandleFunc("/repos/owner/repo/contents/.github", func(w http.ResponseWriter, r *http.Request) {
		ref := r.URL.Query().Get("ref")
		if ref != "develop" {
			t.Errorf("expected ref=develop for .github, got %q", ref)
		}
		json.NewEncoder(w).Encode([]github.ContentEntry{
			{Name: "copilot-instructions.md", Path: ".github/copilot-instructions.md", Type: "file"},
		})
...
func TestScanDirSymlink(t *testing.T) {
	...
	dir := t.TempDir()
	// Place the actual file outside the scanned dir so only the symlink is found.
	otherDir := t.TempDir()
	actualFile := filepath.Join(otherDir, "actual.instructions.md")
...
func TestLoaderSyncRepoUpdatesCache(t *testing.T) {
	...
	mux := http.NewServeMux()
	mux.HandleFunc("/repos/owner/cacherepo/contents/", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/owner/cacherepo/contents/" {
			w.WriteHeader(404)
			return
		}
		json.NewEncoder(w).Encode([]github.ContentEntry{
	...
	mux.HandleFunc("/repos/owner/cacherepo/contents/.github", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode([]github.ContentEntry{
			{Name: "copilot-instructions.md", Path: ".github/copilot-instructions.md", Type: "file"},
		})
	})
...
func TestSyncAllRepos_ContinuesOnError(t *testing.T) {
	...
	mux.HandleFunc("/repos/owner/ok-repo/contents/", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/repos/owner/ok-repo/contents/" {
			w.WriteHeader(404)
			return
		}
		json.NewEncoder(w).Encode([]github.ContentEntry{
	...
	mux.HandleFunc("/repos/owner/ok-repo/contents/.github", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode([]github.ContentEntry{
			{Name: "copilot-instructions.md", Path: ".github/copilot-instructions.md", Type: "file"},
		})
	})
...
func TestFilterByFilePath_EmptyFilePath(t *testing.T) {
all := []Instruction{
{URI: "a", ApplyTo: []string{"**/*.ts"}},
{URI: "b", ApplyTo: nil},
}
got := FilterByFilePath(all, "")
if len(got) != 2 {
t.Errorf("empty filePath should return all %d instructions, got %d", len(all), len(got))
}
}

func TestFilterByFilePath_GlobalAlwaysIncluded(t *testing.T) {
global := Instruction{URI: "global", ApplyTo: nil}
scoped := Instruction{URI: "scoped", ApplyTo: []string{"**/*.go"}}
got := FilterByFilePath([]Instruction{global, scoped}, "src/main.ts")
if len(got) != 1 || got[0].URI != "global" {
t.Errorf("expected only global instruction for .ts file, got %v", got)
}
}

func TestFilterByFilePath_PatternMatch(t *testing.T) {
inst := Instruction{URI: "ts-rules", ApplyTo: []string{"**/*.ts"}}
got := FilterByFilePath([]Instruction{inst}, "src/components/button.ts")
if len(got) != 1 {
t.Errorf("**/*.ts should match button.ts, got %d results", len(got))
}
}

func TestFilterByFilePath_PatternNoMatch(t *testing.T) {
inst := Instruction{URI: "go-rules", ApplyTo: []string{"**/*.go"}}
got := FilterByFilePath([]Instruction{inst}, "src/components/button.ts")
if len(got) != 0 {
t.Errorf("**/*.go should NOT match button.ts, got %d results", len(got))
}
}

func TestFilterByFilePath_MultiplePatterns(t *testing.T) {
// An instruction with multiple patterns is included if any matches.
inst := Instruction{URI: "frontend", ApplyTo: []string{"**/*.ts", "**/*.tsx", "**/*.css"}}
if got := FilterByFilePath([]Instruction{inst}, "src/app.tsx"); len(got) != 1 {
t.Errorf("should match app.tsx via **/*.tsx, got %d", len(got))
}
if got := FilterByFilePath([]Instruction{inst}, "server/main.go"); len(got) != 0 {
t.Errorf("should NOT match main.go, got %d", len(got))
}
}

func TestFilterByFilePath_MixedGlobalAndScoped(t *testing.T) {
instructions := []Instruction{
{URI: "global", ApplyTo: nil},
{URI: "ts-only", ApplyTo: []string{"**/*.ts"}},
{URI: "go-only", ApplyTo: []string{"**/*.go"}},
}
got := FilterByFilePath(instructions, "main.go")
uris := make(map[string]bool)
for _, i := range got {
uris[i.URI] = true
}
if !uris["global"] {
t.Error("global instruction should always be included")
}
if !uris["go-only"] {
t.Error("go-only should be included for main.go")
}
if uris["ts-only"] {
t.Error("ts-only should NOT be included for main.go")
}
}

func TestFilterByFilePath_PreservesOrder(t *testing.T) {
instructions := []Instruction{
{URI: "a", ApplyTo: nil},
{URI: "b", ApplyTo: nil},
{URI: "c", ApplyTo: nil},
}
got := FilterByFilePath(instructions, "any.go")
for i, want := range []string{"a", "b", "c"} {
if got[i].URI != want {
t.Errorf("order violated: got[%d].URI = %q, want %q", i, got[i].URI, want)
}
}
}
...
got := toStringSlice("**/*.go")
if len(got) != 1 || got[0] != "**/*.go" {
t.Errorf("single string → %v, want [**/*.go]", got)
}
}

func TestToStringSlice_StringSlice(t *testing.T) {
input := []interface{}{"**/*.ts", "**/*.tsx"}
got := toStringSlice(input)
if len(got) != 2 || got[0] != "**/*.ts" || got[1] != "**/*.tsx" {
t.Errorf("string slice → %v, want [**/*.ts **/*.tsx]", got)
}
}

func TestToStringSlice_MixedSlice(t *testing.T) {
// Non-string elements in the slice are silently ignored.
input := []interface{}{"**/*.go", 42, nil, "**/*.ts"}
got := toStringSlice(input)
if len(got) != 2 || got[0] != "**/*.go" || got[1] != "**/*.ts" {
t.Errorf("mixed slice → %v, want [**/*.go **/*.ts]", got)
}
}

func TestToStringSlice_EmptySlice(t *testing.T) {
got := toStringSlice([]interface{}{})
if got != nil {
...
got := toStringSlice("**/*.ts,**/*.tsx")
if len(got) != 2 || got[0] != "**/*.ts" || got[1] != "**/*.tsx" {
t.Errorf("comma-separated → %v, want [**/*.ts **/*.tsx]", got)
}
}

func TestToStringSlice_CommaSeparatedWithSpaces(t *testing.T) {
got := toStringSlice("**/*.ts, **/*.go, **/*.py")
if len(got) != 3 || got[0] != "**/*.ts" || got[1] != "**/*.go" || got[2] != "**/*.py" {
t.Errorf("comma+spaces → %v", got)
}
}

func TestSplitGlobPatterns_BraceExpansionPreserved(t *testing.T) {
// Commas inside {} must NOT split the pattern.
got := splitGlobPatterns("**/*.{ts,tsx,js}")
if len(got) != 1 || got[0] != "**/*.{ts,tsx,js}" {
t.Errorf("brace expansion split incorrectly → %v", got)
}
}

func TestSplitGlobPatterns_MixedBraceAndComma(t *testing.T) {
// "**/*.{ts,tsx},**/*.go" → two patterns, brace expansion preserved.
got := splitGlobPatterns("**/*.{ts,tsx},**/*.go")
if len(got) != 2 || got[0] != "**/*.{ts,tsx}" || got[1] != "**/*.go" {
t.Errorf("mixed brace+comma → %v, want [**/*.{ts,tsx} **/*.go]", got)
}
}
