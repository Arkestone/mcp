	...
	srv.AddPrompt(
		&mcp.Prompt{
			...
			Arguments: []*mcp.PromptArgument{
				...
				{Name: "query", Description: "Filter skills by keyword relevance (matches name, description, tags)", Required: false},
			},
		},
		func(ctx context.Context, req *mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
			filtered := filterBySource(scn.List(), req.Params.Arguments["source"])
			filtered = scanner.FilterByQuery(filtered, req.Params.Arguments["query"])
			if len(filtered) == 0 {
...
func registerTools(srv *mcp.Server, scn *scanner.Scanner, opt *optimizer.Optimizer, optimizeDefault bool) {
	...
	type ListInput struct {
		Query    string `json:"query,omitempty"     jsonschema:"Keyword query to filter skills by relevance (name, description, tags). Returns all if empty."`
		Source   string `json:"source,omitempty"    jsonschema:"Filter by source name."`
		FilePath string `json:"file_path,omitempty" jsonschema:"Active file path (e.g. src/auth.ts). Excludes skills whose files: glob patterns do not match."`
	}
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name:        "list-skills",
		Description: "List available skills with metadata. Use query to find relevant skills before calling get-context or get-skill.",
	},
		func(ctx context.Context, req *mcp.CallToolRequest, input ListInput) (*mcp.CallToolResult, ListOutput, error) {
			all := scn.List()
			filtered := scanner.FilterByFilePath(scanner.FilterByQuery(filterBySource(all, input.Source), input.Query), input.FilePath)
			entries := make([]ListEntry, len(filtered))
			for i, s := range filtered {
				entries[i] = ListEntry{URI: s.URI, Name: s.Name, Description: s.Description, Source: s.Source, Tags: s.Tags, Files: s.Files}
			}
			return nil, ListOutput{Total: len(all), Matched: len(filtered), Entries: entries}, nil
		},
	)

	// get-context is the primary agent tool: returns the most relevant skills
	// with full content (including references) merged into a ready-to-inject block.
	type GetContextInput struct {
		Query    string `json:"query"               jsonschema:"Task or context description. Skills are ranked by keyword relevance against name, description, and tags."`
		Source   string `json:"source,omitempty"    jsonschema:"Restrict to a specific source."`
		FilePath string `json:"file_path,omitempty" jsonschema:"Active file path (e.g. src/auth.ts). Excludes skills whose files: glob patterns do not match."`
		Limit    int    `json:"limit,omitempty"     jsonschema:"Maximum number of skills to return (default 5). Use a small value to keep context window size under control."`
	...
	mcp.AddTool(srv, &mcp.Tool{
		Name: "get-context",
		Description: "PRIMARY AGENT TOOL. Returns the most relevant skills with full content and references for the given task context. " +
			"Skills are ranked by keyword relevance (name, description, tags). " +
			"Use limit (default 5) to control how many skills are returned and preserve context window space. " +
			"Call list-skills first to discover what is available, then get-context to load only what you need.",
	},
		func(ctx context.Context, req *mcp.CallToolRequest, input GetContextInput) (*mcp.CallToolResult, GetContextOutput, error) {
			...
			items := make([]SkillItem, len(filtered))
			for i, s := range filtered {
				items[i] = SkillItem{Name: s.Name, Description: s.Description, Tags: s.Tags, Content: s.Content, References: s.References}
			}
	...
	type OptimizeInput struct {
		...
		FilePath string `json:"file_path,omitempty" jsonschema:"Active file path. Excludes skills whose files: glob patterns do not match."`
	...
	mcp.AddTool(srv, &mcp.Tool{Name: "optimize-skills", Description: "Get skills merged via LLM (or concatenated), optionally filtered by keyword query and source."},
		func(ctx context.Context, req *mcp.CallToolRequest, input OptimizeInput) (*mcp.CallToolResult, OptimizeOutput, error) {
			filtered := scanner.FilterByFilePath(scanner.FilterByQuery(filterBySource(scn.List(), input.Source), input.Query), input.FilePath)
