...
func TestParseFrontmatter_Basic(t *testing.T) {
	...
	name, desc, _, _, body := parseFrontmatter(data)
...
func TestParseFrontmatter_QuotedValues(t *testing.T) {
	...
	name, desc, _, _, _ := parseFrontmatter(data)
	if name != "quoted name" {
...
func TestSyncRepo_Basic(t *testing.T) {
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		// skill dir listing
		case strings.HasSuffix(path, "/contents/skills/docker"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/docker/SKILL.md", Type: "file"},
			})
...
func TestSyncRepo_WithReferences(t *testing.T) {
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/contents/skills/docker"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/docker/SKILL.md", Type: "file"},
				{Name: "references", Path: "skills/docker/references", Type: "dir"},
			})
		case strings.HasSuffix(path, "/contents/skills/docker/SKILL.md"):
			w.Write([]byte("---\nname: Docker\ndescription: d\n---\nbody\n"))
...
func TestSyncRepo_MultipleSkills(t *testing.T) {
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/contents/skills/go"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/go/SKILL.md", Type: "file"},
			})
		case strings.HasSuffix(path, "/contents/skills/docker/SKILL.md"):
			w.Write([]byte("---\nname: Docker\ndescription: d\n---\nbody\n"))
...
func TestParseFrontmatter_MultilineIncludesEmptyLines(t *testing.T) {
	...
	data := []byte("---\nname: Test\ndescription: |\n  first\n\n  third\n---\nBody\n")
	_, desc, _, _, _ := parseFrontmatter(data)
...
func TestSyncAllRepos_ContinuesOnError(t *testing.T) {
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.Contains(path, "fail-repo"):
			w.WriteHeader(500)
		case strings.HasSuffix(path, "ok-repo/contents/"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "skills", Path: "skills", Type: "dir"},
			})
		...
		case strings.HasSuffix(path, "ok-repo/contents/skills/my-skill"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
...
func TestSyncRepo_SkillMdFetchFails(t *testing.T) {
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/contents/skills/good-skill"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/good-skill/SKILL.md", Type: "file"},
			})
		case strings.HasSuffix(path, "/contents/skills/bad-skill"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/bad-skill/SKILL.md", Type: "file"},
			})
...
func TestSyncRepo_FileInSkillsDir(t *testing.T) {
	...
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/contents/skills/my-skill"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/my-skill/SKILL.md", Type: "file"},
			})
...
func TestSyncRepo_DirInReferences(t *testing.T) {
	...
	ts := newGitHubTestServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/contents/skills/my-skill"):
			json.NewEncoder(w).Encode([]github.ContentEntry{
				{Name: "SKILL.md", Path: "skills/my-skill/SKILL.md", Type: "file"},
				{Name: "references", Path: "skills/my-skill/references", Type: "dir"},
			})
		case strings.HasSuffix(path, "/contents/skills/my-skill/SKILL.md"):
			w.Write([]byte("---\nname: RefTest\n---\nBody"))
...
func TestParseFrontmatter_LargeBody(t *testing.T) {
	...
	data := []byte("---\nname: Big\n---\n" + large)
	name, _, _, _, body := parseFrontmatter(data)
	if name != "Big" {
		t.Errorf("name = %q", name)
	}
	if body != large {
...
func TestParseFrontmatter_EmptyInput(t *testing.T) {
	name, desc, _, _, body := parseFrontmatter([]byte(""))
	if name != "" || desc != "" || body != "" {
...
skills := []Skill{
{Name: "ts-skill", Files: []string{"**/*.ts"}},
{Name: "global-skill"},
}
got := FilterByFilePath(skills, "")
if len(got) != 2 {
t.Errorf("empty filePath should return all skills, got %d", len(got))
}
}
...
skills := []Skill{
{Name: "ts-skill", Files: []string{"**/*.ts"}},
{Name: "go-skill", Files: []string{"**/*.go"}},
{Name: "global-skill"},
}
got := FilterByFilePath(skills, "src/auth.ts")
if len(got) != 2 {
t.Errorf("expected 2 skills (ts + global), got %d", len(got))
}
for _, s := range got {
if s.Name == "go-skill" {
t.Error("go-skill should be excluded for .ts file")
}
}
}

func TestFilterByFilePath_NoPattern_AlwaysIncluded(t *testing.T) {
skills := []Skill{{Name: "global", Files: nil}}
got := FilterByFilePath(skills, "anything.py")
if len(got) != 1 {
t.Errorf("skill with no Files should always be included, got %d", len(got))
}
}

func TestFilterByFilePath_ParsedFromFrontmatter(t *testing.T) {
dir := t.TempDir()
skillDir := filepath.Join(dir, "skills", "ts-review")
if err := os.MkdirAll(skillDir, 0o755); err != nil {
t.Fatal(err)
}
...
globalDir := filepath.Join(dir, "skills", "global")
...
if len(filtered) != 2 {
t.Errorf("both skills should match .ts file, got %d", len(filtered))
}
filtered2 := FilterByFilePath(all, "main.go")
if len(filtered2) != 1 {
t.Errorf("only global skill should match .go file, got %d", len(filtered2))
}
}
